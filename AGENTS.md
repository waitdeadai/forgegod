...
- OpenAI Codex subscription routing is production-ready on supported native Windows and WSL installs. ForgeGod now treats `codex-only` as a first-class coding surface, and `forgegod evals --matrix openai-live-compare` is the release path for comparing Codex-only against hybrid API+Codex routing when both auth surfaces are linked.
...
- ForgeGod now also has a first-class deterministic harness eval surface:
  ...
  permission denials, completion-gate regressions, loop/worktree behavior,
  and strict-sandbox interface regressions.
...
- ForgeGod now also ships a cheap live OpenAI probe matrix:
  `forgegod evals --matrix openai-live`. It only measures real API/Codex
  surfaces that are actually ready and skips unavailable or temporarily
  quota-limited requested surfaces explicitly instead of faking them.
...
9. **Karpathy Simplicity**: see dedicated section below — minimum code, surgical changes, goal-driven verification.

## Karpathy Coding Principles (from Andrej Karpathy)

Source: https://github.com/forrestchang/andrej-karpathy-skills (MIT)

These reduce common LLM coding mistakes. Tradeoff: bias toward caution over speed — trivial tasks don't need full rigor.

### 1. Think Before Coding
- State assumptions explicitly — ask if uncertain rather than guess.
...
- The current default harness for active product work is **MiniMax M2.7 High Speed as planner/coder** and **`openai-codex:gpt-5.4` as adversarial reviewer/sentinel/escalation**. Apply that pairing across new project configs unless the user explicitly changes it. The architecture remains provider-agnostic, but this is the standing operational default.
