...
- **Native Audit Gate** - `forgegod audit` bridges into `audit-agent`, writes repo-level audit artifacts, exposes specialist security or architecture or plan-risk passes, and lets `forgegod loop` or `forgegod hive` stop before planning when blockers exist.
...
- **Agent Runtime Bridges** - Export first-party Hermes and OpenClaw adapters so those chat runtimes can delegate repo coding into ForgeGod from Telegram, WhatsApp, Discord, and other channels they already own.
...
forgegod run --subagents "Add a /health endpoint to server.py with uptime and version info"
...
# Refresh or inspect repo-level audit artifacts
...
# Export first-party bridges for external chat runtimes
forgegod integrations export-hermes-skill --output ./dist/hermes
forgegod integrations export-openclaw-skill --output ./dist/openclaw-skill
forgegod integrations export-openclaw-backend --output ./dist/openclaw-backend.json

# Machine-friendly chat bridge for Hermes/OpenClaw style runtimes
forgegod bridge chat --runtime hermes --session-id thread-42 --format json "Add a /health endpoint"
...
ForgeGod has one of the deepest memory systems in an open-source coding agent:
...
- `forgegod evals` measures ForgeGod itself: chat UX, approval behavior,
  ...
  `forgegod evals --matrix openai-surfaces` for deterministic OpenAI-first
  routing coverage, and `forgegod evals --matrix openai-live` for cheap real
  API/Codex probes when those auth surfaces are linked. Reports now include
  local trace graders too.
