...
- The currently preferred harness is **MiniMax M2.7 High Speed** as planner/coder and `openai-codex:gpt-5.4` as the adversarial reviewer/sentinel/escalation default, with `zai:glm-5.1` as an additional verified fallback option. ForgeGod stays provider-agnostic; the default is an operational convention, not a hard constraint.
